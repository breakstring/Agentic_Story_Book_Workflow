...
class DalleImageGenerator:
    ...
    def __init__(
        ...
        assert len(
            dalle_configs) > 0, "Invalid DALL-E config. Must contain a valid DALL-E model."
    ...
    def cache_key(self, prompt: str) -> str:
        keys = (prompt, self._model, self._resolution,
                self._quality, self._num_images)
    ...
    def _dalle_client_factory(self, dalle_config: Dict) -> Union[OpenAI, AzureOpenAI]:
        if dalle_config.get("api_type") == "azure":
            return AzureOpenAI(api_key=dalle_config["api_key"],
                               azure_deployment=dalle_config["model"],
                               api_version=dalle_config["api_version"],
                               azure_endpoint=dalle_config["base_url"])
        else:
...
class ImageGeneration(AgentCapability):
    ...
    def add_to_agent(self, agent: ConversableAgent):
        ...
        agent.register_reply(ConversableAgent, self._image_gen_reply,position=self._register_reply_position)
        #agent.register_hook(hookable_method="process_last_received_message", hook=self.process_last_received_message)
        self._text_analyzer_llm_config = self._text_analyzer_llm_config or agent.llm_config
        self._text_analyzer = TextAnalyzerAgent(
            llm_config=self._text_analyzer_llm_config)
    ...
    def _extract_prompt(self, last_message) -> str:
        ...
        analysis = self._text_analyzer.analyze_text(
            last_message, self._text_analyzer_instructions)
        ...
        return {
            "content": [
                ...
                {"type": "image_url", "image_url": {
                    "url": img_utils.pil_to_data_uri(image)}},
            ]
        }
...
def _validate_dalle_model(model: str):
    if model not in VALID_DALLE_MODELS:
        raise ValueError(
            f"Invalid DALL-E model: {model}. Must be in {VALID_DALLE_MODELS}")
